...
*/
void Convert8To32Bit( const uint8_t* pInPalette, uint8_t* pRGBAPalette, const TexFormat_t format )
{
	assert( pInPalette );
	assert( pRGBAPalette );

	uint8_t* pRGBA = pRGBAPalette;
	...
	case TexFormat_t::SPR_INDEXALPHA:
		{
			for( size_t uiIndex = 0; uiIndex < PALETTE_ENTRIES; ++uiIndex, pRGBA += 4 )
			{
				...
				pRGBA[ 1 ] = pInPalette[ uiIndex * 3 + 1 ];
				pRGBA[ 2 ] = pInPalette[ uiIndex * 3 + 2 ];
				pRGBA[ 3 ] = static_cast<uint8_t>( uiIndex );
			}
	...
	case TexFormat_t::SPR_ALPHTEST:
		{
			...
			memset( pRGBAPalette + 255 * 4, 0, sizeof( uint8_t ) * 4 );
...
GLuint UploadMiptex( const miptex_t* pMiptex )
{
	...
	const uint8_t* pBase = reinterpret_cast<const uint8_t*>( pMiptex );

	const uint8_t* pPal = pBase + pMiptex->offsets[ 0 ] + ( ( pMiptex->width * pMiptex->height * 85 ) >> 6 );
	...
	std::unique_ptr<uint8_t[]> image = std::make_unique<uint8_t[]>( uiSize );
	...
	uint8_t* out = image.get();

	const uint8_t	*pix1, *pix2, *pix3, *pix4;

	const uint8_t* pPixelData = reinterpret_cast<const uint8_t*>( pBase + pMiptex->offsets[ 0 ] );
