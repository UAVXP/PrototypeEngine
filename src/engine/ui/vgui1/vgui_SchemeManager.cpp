...
cvar_t *g_CV_BitmapFonts = nullptr;
...
public:
	...
	uint8_t fgColor[4];
	uint8_t bgColor[4];
	uint8_t armedFgColor[4];
	uint8_t armedBgColor[4]; 
	uint8_t mousedownFgColor[4];
	uint8_t mousedownBgColor[4];
...
static uint8_t *LoadFileByResolution( const char *filePrefix, int xRes, const char *filePostfix )
{
	...
	while ( 1 )
	{
		...
		pFile = fileutils::LoadBinaryFile( fname, g_pFileSystem );
...
static void ParseRGBAFromString( uint8_t colorArray[4], const char *colorVector )
{
	int r, g, b, a;
...
CSchemeManager::CSchemeManager( int xRes, int yRes )
{
	...
	char token[1024];
	uint8_t* pFileStart = LoadFileByResolution( "", xRes, "_textscheme.txt" );
	m_xRes = xRes;
	...
	if ( !pFile )
	{
		//TODO: make debug message - Solokiller
		Msg( "Unable to find *_textscheme.txt\n" );
	...
	pFile = tokenization::Parse( pFile, token, sizeof( token ) );
	while ( strlen(token) > 0 && (currentScheme < numTmpSchemes) )
	{
		...
		if ( stricmp( token, "=" ) )
		{
			if ( currentScheme < 0 )
			{
				Msg( "error parsing font scheme text file at file start - expected '=', found '%s''\n", token );
			}
			else
			{
				Msg( "error parsing font scheme text file at scheme '%s' - expected '=', found '%s''\n", tmpSchemes[currentScheme].schemeName, token );
			}
			break;
		}
		...
		if ( !pScheme )
		{
			Msg( "font scheme text file MUST start with a 'SchemeName'\n");
			break;
		}
	...
	// free the file
	fileutils::FreeBinaryFile( pFileStart );
	...
	for ( int i = 0; i < m_iNumSchemes; i++ )
	{
		...
		if ( !m_pSchemeList[i].font )
		{
			...
			if(g_CV_BitmapFonts && g_CV_BitmapFonts->value)
			{
				...
				pFontData = fileutils::LoadBinaryFile( fontFilename, g_pFileSystem );
				if(!pFontData)
					Msg("Missing bitmap font: %s\n", fontFilename);
			}
			...
			fileutils::FreeBinaryFile( pFontData );
