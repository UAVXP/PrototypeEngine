...
bool CopyLump( const dheader_t* const pHeader, const BSPLump lump, DATA& data )
{
	...
	memcpy( data.data, reinterpret_cast<const uint8_t* const>( pHeader ) + iOfs, iLength );
...
std::unique_ptr<dheader_t> LoadBSPFile( const char* const pszFileName )
{
	...
	std::unique_ptr<dheader_t> header( reinterpret_cast<dheader_t*>( new uint8_t[ size ] ) );

	dheader_t* pHeader = header.get();

	const auto readCount = file.Read( header.get(), size );
